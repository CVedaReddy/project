...
void delay(ms)                                          // delay's printing the output statements by milliseconds
{
...
int logprompt()                                        /// OPENING initial window for user 
{
    int n;
    gotoxy(9,6);
    ...
    { printf("=");}
    gotoxy(38,13);
    printf("** Enter D TO LOGIN **");
    gotoxy(36,15);
    printf("** PRESS P TO REGISTER **");
    gotoxy(35,17);
    printf("** PRESS Q TO SEE ABOUT **");
    gotoxy(37,19);
...
void escape()                                              /// ESC command to end program
{
    system("cls");
...
void about()
{
    system("cls");
    printf("\n\n\n\n");
    printf(" ================  *   ===  *  === * M Y . J O U R N A L . S P A C E * ===  *  ===  * ==================== \n\n\n");
    printf("     ============================================================================================\n\n");

    printf("    ABOUT PROJECT :\n\n");

    printf("    A BRIEF OVERVIEW OF 'MY JOURNAL SPACE '\n");
    printf("    PROJECT BY veda,varshitha and mohitha \n ");
    printf("    This Project is fully coded in C Language using concepts like: FILE HANDLING , FUNCTION, POINTERS, HEADER FILE, TIME etc ...\n\n");
    printf("    My Journal Space provides a user-friendly interface which he/she can use to access their text from\n");
    printf("    It has a password protected, and a USER can either Login or Register themselves accordingly\n");
    printf("      login()   performs Login task \n");
    printf("      userreg() performs REGISTERATION task \n\n");
    printf("    gotoxy() helps to set co-ordinate on console \n");
    printf("    delay()  is used in a manner such that it provides a little animation and to delay printing the output statements \n\n");
    ...
    printf("                'NOTEPAD_WRITE' has all functionalities of OVERWRITE MODE\n");
    printf("    ADD MODE  - It add more data after previously stored data.\n");
    printf("                'NOTEPAD_ADDTEXT' has all functionalities of ADD MODE\n");
    printf("    DEMO MODE - It provides a user-friendly interface where a user can check how everything works.\n");
    printf("                'NOTEPAD_DEMO' has all functionalities of DEMO MODE\n\n\n");
    ...
    printf("    THEY ARE PROVIDE IN 'NOTEPAD_HELP' FILE\n\n");
...
{   gotoxy(22,12);
    ...
    delay(10);}
    gotoxy(27,14); 
    printf("---x---xx--xx- NO USERFILE EXIT -xx--xx---x---");
    gotoxy(38, 16);
     printf("-- REGISTER FIRST --");
    gotoxy(32, 18);
     delay(800);
    printf("---- PRESS P FOR REGISTERATION ----\n");
    while(1){
        int k=getch(); 
    if(k==80)
    userreg();}   /// userreg() is defined in REGISTER_USER file
}
...
{   fptr = fopen("userrecord.bin", "rb");         ///  opens file in read mode
    ...
    {   fread(&use, sizeof(use), 1,fptr);
        ...
        if(chku==1 && chkp==1)
        {   found=1; fclose(fptr);                /// gotoxy() is defined in FUNCTIONS file
            for(int i=0; i<53; i++)
            gotoxy(22,12);
            for(int i=0; i<53; i++)
            { printf("-");delay( 10); }           /// delay() is defined in FUNCTIONS file
            gotoxy(27,14);
            ...
            { if(k==13) 
            notepadinput(uname);
             k= getch();}   /// notepadinput() is defined in NOTEPAD file
        }   else continue;
    }
...
void userchk(FILE *usercheck, char uname[])                      ///  CHECKS WEATHER USERNAME ALREADY EXISTS  ///
{
    ...
    while(!feof(usercheck))
    {
        ...
        {   fclose(usercheck); gotoxy(22,10);
            ...
            printf("---- PRESS D TO LOGIN ----");
            gotoxy(36,22); delay(600);
            printf("---- PRESS P TO TRY AGAIN ----");
            gotoxy(22,24);
            for(int i=0; i<61; i++)
            { printf("-"); delay(10) ;}printf("\n");
            getch();
            while(1)
            {   int k=getch(); if(k==68) login();
                else if(k==80) userreg();
            }
        } else continue;
    }
}
...
void username_valid()                                           ///  CHECKS WEATHER USERNAME IS VAILD OR NOT  ///
{
    ...
    {   gotoxy(22,18);
        ...
        getch();
        while(1){ int k=getch(); if(k==80) userreg();}
    }
    else return;
}
...
{   FILE *usercheck= fopen("userrecord.bin", "rb");
    ...
    if(j==1)
    {
        ...
        {   gotoxy(40, 16);
            ...
            while(1){int n=getch(); if(n==80) userreg();}
        }
    }
    else if(j==2)
    {   gotoxy(30,12);
        for(int i=0; i<46; i++)
        {printf("-");delay(20);}
        gotoxy(45,14); printf("WEAK PASSWORD");
        gotoxy(24, 16);
...
void notepadinput(char name[])                      /// MAIN NOTEPAD FUNCTION
{
    ...
    gotoxy(34,14); printf(">>> PRESS D FOR HELP <<<");
    gotoxy(34,16); printf(">>> PRESS  P FOR DEMO <<<");
    gotoxy(31,18); printf(">>> PRESS Q TO DISPLAY TEXT <<<");
    gotoxy(33,20); printf(">>> PRESS R TO ADD TEXT <<<\n\n");
    gotoxy(29,22);printf(">>>> PRESS S TO OVERWRITE FILE <<<<\n\n");
    ...
    {   int n=getch();
        switch(n)
        {
            case 68 : notehelp();break;
            case 80 : notedemo();break;
            case 81:  notedisplay();break;
            case 82:  noteadd(); break;
            case 83:  notewrite(); break;
...
void enterhit_demo()                                        /// d e m o _ e n t e r _ h i t _ c a l l
{
    ...
    gotoxy(27,f+8); printf("---||--||- press D to go back -||-||-||---");
    gotoxy(27,f+10); printf("||-||-||- press P to test again -||-||-||");
    gotoxy(30,f+12);
    for(int i=0; i<19; i++)
    {
        printf("\\"); delay(10);
    ...
    while(1)
    {
        if(f==68) notepadinput(uname);
        else if(f==80) enterhit_demo();
        f=getch();
    }
}
...
void notedemo()                                           ///   M A I N _ D E M O _ F U N C T I O N
{
    ...
    gotoxy(30,19); printf(">>>>>>  PRESS D TO GO BACK  <<<<<");
    ...
    while(1)
    {
        ...
        else if(k==68){ notepadinput(uname); break;}
...
void enterhit_display()
{
    ...
    for(int i=0; i<95; i++)
    { printf("-"); delay(5);}
    gotoxy(27,j+2); printf("---||--||- press D to add text -||-||-||---");
    gotoxy(26,j+4);printf("---||--||-  press P to overwrite  -||-||-||---");
    gotoxy(27,j+6); printf("---||--||- press Q to go back -||-||-||---");
    ...
    {
        int k= getch();
        if(k==68) { noteadd(); break;}
        else if(k==80){ notewrite(); break;}
        else if(k==81) {notepadinput(uname); break;}
        else if(k==ESC) {escape(); break;}
    }
}
...
{   system("cls"); gotoxy(14,4);
    ...
    gotoxy(30,17); printf(">>>>>>  PRESS D TO GO BACK  <<<<<"); gotoxy(23,19);
    for(int i=0; i<26; i++)
...
void noteadd()
{
    ...
    for(int i=0; i<95; i++)
    { printf("-"); delay(5);}
    gotoxy(25,f+2); printf("f---||--||- press D to add timstamp -||-||-||---");
    gotoxy(28,f+4);    printf("---||--||- press P to add text -||-||-||---");
    gotoxy(29,f+6);     printf("---||--||- press Q to go back -||-||-||---");
    ...
    {
        int k= getch();
        if(k==68)       { notewrite_timestamp(1, f+12, 2, text); break;} ///        FUNCTION YET TO ADD
        else if(k==80)  { notewrite_timestamp(2, f+12, 2, text); break;}    ///         FUNCTION YET TO ADD
        else if(k==81)  { notepadinput(uname); break;}
        else if(k==ESC) { escape(); break;}
    }
}
...
void enterhit_write()
{
    ...
    {
        int k= getch();
        if     (k==68)  { notewrite_timestamp(1, f+12, 1, text); break;} ///        FUNCTION YET TO ADD
        else if(k==80)  { notewrite_timestamp(2, f+12, 1, text); break;}    ///         FUNCTION YET TO ADD
        else if(k==81)  { notepadinput(uname); break;}
        else if(k==ESC) { escape(); break;}
    }
}
...
void notewrite()
{
    ...
    gotoxy(30,17); printf(">>>>>>  PRESS D TO GO BACK  <<<<<");
    gotoxy(28,19);printf(">>>>>>   PRESS P TO ADD TEXt   <<<<<");
    gotoxy(23,21);
    for(int i=0; i<26; i++)
